...
/*                                                   +#+                      */
/*   Created: 2020/02/01 14:55:40 by dsaripap      #+#    #+#                 */
/*   Updated: 2020/02/01 14:55:41 by dsaripap      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int					ft_errormanagement(int error, t_data *input_data)
{
	if (error == 1)
	{
		printf("\nPlease enter the correct number of arguments as shown in the usage below \n");
		printf(ANSI_COLOR_RED"usage: ft_contrast -f [image_name.pgm] -c [contrast_level_number] -o [output_filename] \n"ANSI_COLOR_RESET);
	}
	else if (error == 2)
	{
		printf("\nPlease enter the correct type of flags as shown in the usage below \n");
		printf(ANSI_COLOR_RED"usage: ft_contrast -f [image_name.pgm] -c [contrast_level_number] -o [output_filename] \n"ANSI_COLOR_RESET);
	}
	else if (error == 3)
	{
		printf("\nWrong contrast level \n");
		printf(ANSI_COLOR_RED"Please enter a valid contrast level (number between 0 and 100) \n"ANSI_COLOR_RESET);
	}
	else if (error == 4)
